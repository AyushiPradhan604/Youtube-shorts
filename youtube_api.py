...
class YouTubeAPI:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.api_key = os.getenv(config.get('api_key_env', 'YOUTUBE_API_KEY'))
    ...
    async def search_shorts(
        ...
        Returns:
            List of short video metadata dictionaries
        """
        ...
        try:
            ...
            while len(all_videos) < max_results:
                ...
                remaining = min(50, max_results - len(all_videos))  # YouTube API max is 50 per request
                ...
                search_request = self.youtube.search().list(
                    ...
                    videoDuration='short',  # Try to get short videos
                    order='relevance'
                )
                ...
                shorts = [video for video in videos_detail if self._is_short_video(video)]
    ...
    def _is_short_video(self, video: Dict[str, Any]) -> bool:
        """Check if video qualifies as a Short (duration <= 60 seconds)"""
        duration = video.get('duration')
        return duration is not None and duration <= 60
    
    async def _execute_with_retry(self, request) -> Dict[str, Any]:
        """Execute YouTube API request with retry logic for quota management"""
        for attempt in range(self.max_retries):
            try:
                # Execute synchronous request in thread pool
                ...
                if e.resp.status == 403:  # Quota exceeded
                    if attempt < self.max_retries - 1:
                        self.logger.warning(
                            f"YouTube API quota exceeded. Retrying in {self.quota_retry_delay} seconds..."
                        )
